...
import com.psb.devquery.ViewProfileActivity;
...
public class HomeFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        //parsePost();
        if(GlobalData.getInstance().firstLoad){
            post = GlobalData.getInstance().post;
            load();
            GlobalData.getInstance().firstLoad = false;
        }
        else parsePost();
        //Toast.makeText(getActivity(),GlobalData.getInstance().post.get(0).getPostOwner()+"",Toast.LENGTH_LONG).show();
        return v;
    ...
    public void load(){
        GlobalData.getInstance().post = post;
        postAdapter = new PostAdapter(post,getActivity());
        rcv.setAdapter(postAdapter);
//        Toast.makeText(getActivity(),"interface ok",Toast.LENGTH_LONG).show();
